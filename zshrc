...
RR() {
  docker run -ti --rm --hostname=vl-ds-container --cpus=$(( $( (command nproc 2>/dev/null) || sysctl -n hw.cpu) - 1.5)) -v "$(pwd):/app" -w /app vl-ds R "$@"
}
...
alias ipython='ipython --pdb --no-autoindent --no-pprint --no-banner --no-confirm-exit '
