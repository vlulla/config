...
updatecondaenvs() {
  ...
  for env in "${envs[@]}"; do
    ...
    printf "%b\n" "Updated micromamba environment:   ${bold}${green}${env}${reset}"
  done
}
...
export PS1=$'\n%B%n@${PUBLIC_HOSTNAME}%(2L. [SHLVL: %L].): %F{cyan}%(8~|%-1~/.../%6~|%7~)%f%b\n%B[%D{%Y.%m.%d}] \$(git_branch_info)%(?.. %Ux %?%u) %#%b '
...
[[ -d "${HOME}/code/J/j9.7" ]] && export JHOME="${HOME}/code/J/j9.7"
...
alias e='vim --ttyfail'
( command -v julia >/dev/null 2>&1 ) && alias julia='julia --banner=no --color=no --procs=auto '
...
alias view="view -c 'map <SPACE> <C-F>zz' -c 'map b <C-B>zz' -c 'map q :q<CR>' "
